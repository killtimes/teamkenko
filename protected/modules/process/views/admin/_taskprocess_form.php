...
<div class="<?php echo ($model->hasErrors('task_id')) ? 'has-error' : '' ?> form-group">        
    <label class="col-sm-3 control-label required">Task <span class="required">*</span></label>
    <div class="col-sm-7"> 
        <?php
...
<div class="<?php echo ($model->hasErrors('assign_id')) ? 'has-error' : '' ?> form-group">        
    <label class="col-sm-3 control-label required">Assign To <span class="required">*</span></label>
    <div class="col-sm-7"> 
        <?php
...
<div class="<?php echo ($model->hasErrors('priority')) ? 'has-error' : '' ?> form-group">        
    ...
    <div class="col-sm-6"> 
        ...
        $this->renderPartial('webroot.themes.' . Yii::app()->theme->name . '.views.includes._select_box', array(
            ...
            'data' => CMap::mergeArray(array('' => ''), TaskProcess::itemAlias('Priority')),
            'extra' => array(
                'pluginOptions' => array(
...
<div class="<?php echo ($model->hasErrors('send_mail_recipients')) ? 'has-error' : '' ?> form-group">
    ...
    <div class="col-md-7">
        ...
        <a href="javascript:;" onclick="$('#s2id_TaskProcess_send_mail_recipients').select2('val',$('#s2id_TaskProcess_alert_recipients').select2('val'))">List user above</a>
...
<?php if ($model->stage == TaskProcess::STAGE_REJECTED) { ?>
    <div class="<?php echo ($model->hasErrors('priority')) ? 'has-error' : '' ?> form-group">        
        <label class="col-sm-3 control-label">Re-assign</label>
        <div class="col-sm-7"> 
            <?php
            $this->widget('yiiwheels.widgets.switch.WhSwitch', array(
                'name' => 'TaskProcess[re_assign]'
            ));
            ?>
        </div>
    </div>
