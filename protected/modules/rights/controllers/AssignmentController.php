...
class AssignmentController extends RController {
    ...
    public function actionRevoke() {
        ...
        if (Yii::app()->request->isPostRequest === true) {
            ...
            if (!(Yii::app()->getModule('rights')->isSystemUser($user->username) && $itemName=='Admin') &&
                    !($user->superuser == 1 && $itemName=='Admin')) {
                ...
                $item = $this->_authorizer->attachAuthItemBehavior($item);
                // Set flash message for revoking the item
                Yii::app()->user->setFlash($this->module->flashErrorKey, Rights::t('core', 'Can not revoke permission :name on this user.', array(':name' => $item->getNameText()))
                );
            }
