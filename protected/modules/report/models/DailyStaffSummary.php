...
class DailyStaffSummary extends CFormModel
{

    public function getSummary()
    {
        $data = Yii::app()->db->createCommand()
            ->select('tp.assign_id, tp.stage as stage, count(tp.id) as total')
            ->from('TaskProcess tp')
...
//        $today = DateTime::createFromFormat('d-m-Y', '23-11-2016');
    ...
    public function getDailyByStaff($staff, &$date)
    {
...
//        $today = DateTime::createFromFormat('d-m-Y', '23-11-2016');
        ...
        $data = Yii::app()->db->createCommand()
            ...
            ->from('TaskProcess tp')
            ->join('Process t', 'tp.process_id=t.id')
            ->where('t.status=1 and tp.status=1 and tp.stage<>0 and t.assign_id=:assign_id and tp.assign_date>=:r1 and tp.assign_date<:r2', array(
                ':assign_id' => $staff,
                ':r1' => $r1,
                ':r2' => $r2
            ))
            ->group(array('tp.stage'))
    ...
    public function byDailyStaff($staff, &$date)
    {
