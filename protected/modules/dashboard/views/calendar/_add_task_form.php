<?php if (Yii::app()->user->hasFlash('successMessage')) { ?>
    <?php
    Yii::app()->clientScript->registerScript('refresh-task', "
        $(document).off('hidden.bs.modal').on('hidden.bs.modal', function (e) {
            if (e.target.id === 'mdAddTask') {
                DashboardPage.loadTaskByDate('{$date}');
            }
        });
    ");
    ?>
...
<div class="form-group">
    ...
    <div class="<?php echo ($model->hasErrors('customTaskName')) ? 'has-error' : '' ?> form-group">        
        ...
        <div class="col-md-7"> 
            ...
            $this->widget('yiiwheels.widgets.typeahead.WhTypeAhead', array(
                ...
                'pluginOptions' => array(
                    'local' => array_keys(CHtml::listData(Task::model()->findAll(), 'name', 'name'))
                )
            ));
            ?>
    ...
    <div class="form-group">        
        <label class="col-md-3 control-label required">Start time <span class="required">*</span></label>
        <div class="col-md-7"> 
            <?php
            $this->widget('yiiwheels.widgets.timepicker.WhTimePicker', array(
                'name' => 'TaskProcess[start_time]',
                'pluginOptions' => array(
                    'showMeridian' => false,
                    'defaultTime' => 'value'
                )
    ...
    <div class="<?php echo ($model->hasErrors('assign_id')) ? 'has-error' : '' ?> form-group">        
        ...
        <div class="col-md-7"> 
            ...
            <?php if (Yii::app()->user->hasRole('Manager') || Yii::app()->user->hasRole('Admin')) { ?>
                <?php
