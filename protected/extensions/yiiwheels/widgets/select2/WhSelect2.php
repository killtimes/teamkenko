...
class WhSelect2 extends CInputWidget {
    ...
    public function registerClientScript() {
        ...
        $cs->registerCssFile($assetsUrl . '/css/select2.css', '', 100);
        $cs->registerCssFile($assetsUrl . '/css/select2-bootstrap.css', '', 101);
        $cs->registerScriptFile($assetsUrl . '/js/select2.js');


        if ($this->language) {
            $cs->registerScriptFile(
                    $assetsUrl . '/js/locale/select2_locale_' . $this->language . '.js', CClientScript::POS_END
            );
        }

        /* initialize plugin */
        $selector = '#' . TbArray::getValue('id', $this->htmlOptions, $this->getId());
        $this->getApi()->registerPlugin('select2', $selector, $this->pluginOptions, CClientScript::POS_END);
        $this->getApi()->registerEvents($selector, $this->events, CClientScript::POS_END);

        if ($this->readonly) {
            $script = "jQuery('{$selector}').select2('readonly',{$this->readonly});";
            $id = __CLASS__ . '#Plugin' . TbApi::$counter++;
